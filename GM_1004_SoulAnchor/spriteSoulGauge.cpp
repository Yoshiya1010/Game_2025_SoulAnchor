...
{
   //現在の値から　目標の値に増減する
    const float ratio = m_Value / 100.0f; 
    const float halfH = m_Height * 0.5f;
    const float visH = m_Height * ratio;
...
static inline float ClampHundred(float v) {
...
{
    m_TargetValue = ClampHundred(percent);
}
