...
static ID3D11RasterizerState* gRS_CullBack = nullptr;
static ID3D11RasterizerState* gRS_CullNone = nullptr;
...
void Renderer::Init()
{
	...
	// デバイス、スワップチェーン作成（スワップチェーンはフロントバッファとバックバッファを切り替えるためのモノ）
	...
	// デプスステンシルビュー作成（デプスステンシルバッファにアクセスするための窓口みたいなもの）
	...
	// gRS_CullBackを作成
	m_Device->CreateRasterizerState(&rasterizerDesc, &gRS_CullBack);

	// gRS_CullNoneも作成
	rasterizerDesc.CullMode = D3D11_CULL_NONE;
	m_Device->CreateRasterizerState(&rasterizerDesc, &gRS_CullNone);

	// デフォルトはバックカリング
	m_DeviceContext->RSSetState(gRS_CullBack);
	...
	// サンプラーステート設定（テクスチャからどうやって色を取り出すか）
...
{
	m_DeviceContext->UpdateSubresource(m_CameraBuffer, 0, NULL, &Parameter, 0, 0);
}
...
void Renderer::CreateLineVertexShader(ID3D11VertexShader** VertexShader, ID3D11InputLayout** VertexLayout, const char* FileName)
{
	FILE* file;
	long int fsize;
	...
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "NORMAL",   0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0 },  // 追加
		{ "COLOR",    0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 24, D3D11_INPUT_PER_VERTEX_DATA, 0 },  // 24に変更！
		{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 40, D3D11_INPUT_PER_VERTEX_DATA, 0 }  // 追加
	};
	UINT numElements = ARRAYSIZE(layout);
