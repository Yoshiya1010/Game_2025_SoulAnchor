...
BrightExtractParams PostProcessManager::m_BrightExtractParams = { 0.8f, 1.5f, 0.0f, 0.0f };
...
void PostProcessManager::Init()
{
    ...
    Renderer::CreatePixelShader(&m_BrightExtractPS, "shader/BrightExtractPS.cso");
    Renderer::CreatePixelShader(&m_BlurPS, "shader/SimpleBlurPS.cso");

    // BrightExtractパラメータバッファ作成
    D3D11_BUFFER_DESC brightParamBufferDesc{};
    brightParamBufferDesc.ByteWidth = sizeof(BrightExtractParams);
    brightParamBufferDesc.Usage = D3D11_USAGE_DEFAULT;
    brightParamBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    device->CreateBuffer(&brightParamBufferDesc, nullptr, &m_BrightExtractParamBuffer);
...
void PostProcessManager::Uninit()
{
    ...
    if (m_BrightExtractPS) m_BrightExtractPS->Release();
    if (m_BrightExtractParamBuffer) m_BrightExtractParamBuffer->Release();
}
...
void PostProcessManager::ApplyEffect(PostEffectType type)
{
    ...
    case PostEffectType::BLOOM:
        UpdateBrightExtractParams();
        context->PSSetConstantBuffers(0, 1, &m_BrightExtractParamBuffer);
        context->PSSetShader(m_BrightExtractPS, nullptr, 0);
        break;
...
void PostProcessManager::UpdateBrightExtractParams()
{
    ID3D11DeviceContext* context = Renderer::GetDeviceContext();
    context->UpdateSubresource(m_BrightExtractParamBuffer, 0, nullptr, &m_BrightExtractParams, 0, 0);
}